...
For every root or delegated agent, apply [status and completion communication](../docs/STATUS-AND-COMPLETION.md). `PENDING.md` owns human decisions/actions and macro project gaps; `TASK-GRAPH.md` owns technical order, dependencies, and execution. Routine progress/step updates stay concise: state the material result or evidence, any blocker or human action, and what follows, with inspectable paths when useful. Do not render or reconstruct the complete status view for every routine update. Explicit status/pending requests and milestone closeouts use the full [`harness.status/v1`](../docs/contracts/STATUS.md) view, including human/macro pending items and active/ready/blocked graph nodes. Passing tasks are marked `completed` and unlock the next node immediately; assurance review is automatic, non-blocking, and never a renewed human approval request.
