...
`VERSION` is the single version value shared by all three bundles; profile names are suffixes, not independent versions. `0.1.0` is the initial public source version, `0.2.0` adds contained installation plus continuous-delivery governance, and `0.3.0` adds executable status reporting, auditable focused re-review boundaries, hostile governance mutations, and GitHub-compatible overview media. A future approved release changes `VERSION` once, validates all profiles, and may attach the three archives to one GitHub Release. Release automation remains an open decision.
