...
Source candidate `0.7.2` adds client milestone pauses, progressive feature scope, explicit completion conditions, and runtime evidence gates; public-index verification is pending. Legacy JSON nodes remain compatible, while table-only Markdown requires an executable JSON block before ready/active/completed transitions.
