...
Release `0.7.0` makes execution adaptive and measurable: lane, assurance, and artifact shape are independent; resume probes real state first; preflight blocks missing prerequisites; handoff/review files require a real consumer; planned units target 15–30 active minutes and use a proportional test ladder; scheduling maximizes safe concurrency; graph transitions use compare-and-swap atomic replacement; and append-only events/metrics expose ceremony cost and gate value.

`VERSION` is the single version value shared by all three bundles; profile names are suffixes, not independent versions. `0.1.0` is the initial public source version, `0.2.0` adds contained installation plus continuous-delivery governance, `0.3.0` adds executable status reporting and bounded reviews, `0.4.0` adds frontend/learning/context routing, `0.5.x` adds the installable CLI and hackathon delivery, `0.6.x` adds feature discovery, TDD, native dispatch, parallel scheduling, and graph readiness, and `0.7.0` adds adaptive execution, real-state-first resume, preflight, compact artifacts, runtime state/event/metric primitives, and benchmark isolation. A future approved release changes `VERSION` once and validates all profiles.
