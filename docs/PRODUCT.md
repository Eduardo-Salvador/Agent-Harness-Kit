...
The current source candidate is `0.7.2`, adding accompanied delivery, progressive scope closure, and executable completion/approval gates. Its publication verification is pending. The previously verified public-index scope is version `0.7.1` under `agent-harness-kit-cli`. It adds adaptive first-run resolution of architecture, folder organization, and optional coding conventions to native activation for Codex and Claude Code. The public package has been smoke-tested through a clean contained installation, managed root bridges, the packaged project-shape contract, `doctor`, and the bundled validator. It retains lane/assurance/shape selection, evidence-first resume, prerequisite preflight, consumer-driven artifacts, proportional testing, maximum-cardinality scheduling, atomic graph transitions, hash-chained events, and run metrics. It does not include an unattended daemon, provision credentials, integrate branches, deploy, or publish notes independently. Remaining capability and automation decisions are tracked in [OPEN-DECISIONS.md](../OPEN-DECISIONS.md).
