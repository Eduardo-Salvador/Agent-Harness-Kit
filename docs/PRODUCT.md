...
The current public source and PyPI scope is version `0.7.0` under `agent-harness-kit-cli`. It includes native activation for Codex and Claude Code plus adaptive lane/assurance/shape selection, evidence-first resume, prerequisite preflight, consumer-driven artifacts, proportional testing, maximum-cardinality scheduling, atomic graph transitions, hash-chained events, and run metrics. Its public-index package has been smoke-tested through a clean contained installation, managed root bridges, `doctor`, and the bundled validator. A profile may also be installed under `agent-harness-kit/` with minimal root bridges. It does not include an unattended daemon, provision credentials, integrate branches, deploy, or publish notes independently. Remaining capability and automation decisions are tracked in [OPEN-DECISIONS.md](../OPEN-DECISIONS.md).
