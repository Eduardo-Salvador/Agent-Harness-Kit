...
The current source scope is version `0.7.1`; the latest public-index release remains `0.7.0` until the hotfix publication gate completes. The `0.7.1` scope adds adaptive first-run resolution of architecture, folder organization, and optional coding conventions to native activation for Codex and Claude Code. It retains lane/assurance/shape selection, evidence-first resume, prerequisite preflight, consumer-driven artifacts, proportional testing, maximum-cardinality scheduling, atomic graph transitions, hash-chained events, and run metrics. A profile may be installed under `agent-harness-kit/` with minimal root bridges. It does not include an unattended daemon, provision credentials, integrate branches, deploy, or publish notes independently. Remaining capability and automation decisions are tracked in [OPEN-DECISIONS.md](../OPEN-DECISIONS.md).
