...
The source candidate is `0.7.3`, adding accompanied, autonomous, and hackathon delivery presets with a read-only CLI inspector. Its publication verification is pending. The previously verified public-index version is `0.7.2` under `agent-harness-kit-cli`. It adds accompanied delivery with client milestone pauses, progressive feature scope closure, explicit task completion conditions, and executable scope/product/verification gates. The public package was downloaded without cache and checked through a clean contained installation, both managed root bridges, `doctor`, the embedded validator, and all 20 delivery-gate tests. It retains adaptive lane/assurance/shape selection, evidence-first resume, prerequisite preflight, consumer-driven artifacts, proportional testing, collision-free scheduling, atomic transitions, events, and metrics. These checks validate recorded evidence, not authentic execution or human identity. Legacy JSON nodes remain compatible; table-only Markdown requires migration before dispatch/completion. The Kit does not run as an unattended daemon, provision credentials, integrate branches, deploy, or publish notes independently. Remaining capability decisions are tracked in [OPEN-DECISIONS.md](../OPEN-DECISIONS.md).
