...
The current public source scope is version `0.7.0` and includes native activation for Codex and Claude Code plus adaptive lane/assurance/shape selection, evidence-first resume, prerequisite preflight, consumer-driven artifacts, proportional testing, maximum-cardinality scheduling, atomic graph transitions, hash-chained events, and run metrics. The latest previously verified PyPI publication is `0.5.3` under `agent-harness-kit-cli`; `0.7.0` remains source-ready until publication and public-index smoke testing. A profile may also be installed under `agent-harness-kit/` with minimal root bridges. It does not include an unattended daemon, provision credentials, integrate branches, deploy, or publish notes independently. Remaining capability and automation decisions are tracked in [OPEN-DECISIONS.md](../OPEN-DECISIONS.md).
