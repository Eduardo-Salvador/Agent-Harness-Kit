...
- New nodes also record `planning_mode`, `implementation_plan`, `plan_step`, `target_minutes`, orthogonal `assurance`, `artifact_policy`, `handoff_consumer`, and `test_ladder`. Planned nodes pin a ready plan revision and normally target 15–30 active minutes; `inline-simple` nodes use `none`, `inline`, and at most five minutes.
