...
3. For work that is not `direct-trivial`, classify it using the simple-task gate. A simple task gets one compact inline spec; every other implementation outcome requires `../../../harness-state/plans/PLAN-<id>.md` from `../../../harness/templates/IMPLEMENTATION-PLAN.md`. Select `evidence_profile: graph-only` only for an inline-simple, deterministic, low-risk, non-behavior task that passes every graph-only exclusion; otherwise use `handoff-review`.
