...
4. Create a `harness.parallel-dispatch/v1` record from `PARALLEL-DISPATCH` in the [runtime template catalog](../TEMPLATES.md) as `reserving` against the expected graph revision. For every selected node reserve a distinct lease, isolation, task SPEC, model-dispatch route, workstream/role, and context slot. A stale revision aborts before launch.
