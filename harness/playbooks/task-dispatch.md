...
7. Create/resume the declared context through the adapter, persist its reference, and send a notification pointing to the task artifact. The specialist loads only declared context, starting with the node's `read_set`; `write_set` remains the exclusive lease and `impact_set` bounds regression checks. Record `context_provenance` with a pinned source revision when these hints come from source inspection or an approved fresh repository index. Different workstreams cannot share one context except a bounded integration node.
