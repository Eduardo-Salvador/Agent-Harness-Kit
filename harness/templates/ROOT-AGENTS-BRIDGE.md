...
# STOP: Agent Harness Kit mandatory first-response gate
...
3. If context is absent or not approved, stop. Do not answer the substantive project request, propose a solution, or perform a broad scan. Start the first-run discovery interview automatically.
...
6. Treat model memory and prior conversations as unverified. They are not project facts unless the current user message or an approved project artifact supplies them.
7. Never claim information was “registered mentally.” Only call it recorded when a draft artifact was written, and cite its path/revision.
8. Before sending, self-check: if the response contains a proposal or more than one discovery question, replace it with the restricted handshake above.
9. If context is approved, follow the harness resume, pending, graph, status, authority, and capability rules.
