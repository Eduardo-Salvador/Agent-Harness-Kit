...
- On first contact with an uninitialized project, stop before answering the substantive request. Use only the localized first-response handshake: brief kit-active welcome, one short explanation that it organizes project context, pending work, and verifiable execution, discovery-before-proposals statement, notice that standard delivery or the faster hackathon mode for a time-boxed MVP/demo is available, then exactly one highest-leverage unanswered question. Never lead with inferred company facts, branding, colors, a product or feature proposal, design, architecture, stack, plan, status, or graph. Model memory and prior conversations are unverified until supported by the current message or an approved project artifact.
